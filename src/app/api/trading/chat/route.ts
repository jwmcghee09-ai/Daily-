...
export async function POST(request: NextRequest) {
  ...
  if (!groqKey) return NextResponse.json({ error: "GROQ_API_KEY not configured on server" }, { status: 503 });
  ...
  const messages: Message[] = (body.messages as Message[]).map(m => ({
    role: m.role,
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    if (!res.ok) {
      ...
      return NextResponse.json({ error: `Groq error: ${err.slice(0, 400)}` }, { status: 502 });
    }
